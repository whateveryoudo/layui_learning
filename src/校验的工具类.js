		...
		success:function(data){
			...
			if(data){
				if(data.headerInfo.to404 && data.headerInfo.to404===true){
					window.location.href = 'http://image2.chinatelecom-ec.com/client/wap/common/page/error_404.html?errorCode=20001';
					return false;
				}else{
...
function showLoginWaitCurr(top,left){
	...
	if(isNull(page.find('#loginWait')) || page.find('#loginWait').length <= 0){
		var loadingHtml =	'\
			...
			<img src="http://image2.chinatelecom-ec.com/client/wap/common/images/load.gif" style="position:relative;top:29%;width:25%;"></div>\
			</div>\
			</div>';
...
function getServerDate(func,t){
	//当年年月日及完整时间戳
	var serviceDate  = '',getDate = '',date_time={};
	//获取相应头
	var xhr = new XMLHttpRequest();
	xhr.open("HEAD",location.href+(t || ''),true);
	xhr.onreadystatechange=function(){
		if( xhr.readyState == 4 && xhr.status == 200 ){
			serviceDate = xhr.getResponseHeader("Date");
			if(isNull(serviceDate)){
 ...
 * var opt = {
 *			cla:'backBtn',
 *			url:'http://image2.chinatelecom-ec.com/client/wap/common/images/icon_back.png',
 *			imgCla:''
 *	}
 */
function goBack(set){
	...
	if(content.children('.'+opt.cla).length ===0){
		var html = '<div class="'+opt.cla+'" onclick="history.back();"><img width="45px" class="'+opt.imgCla+'" src="'+opt.url+'" /></div>';
		content.append(html);
	}
}
/**
...
function goTop(){
 	...
 	var html = '<div class="toTopBtn" onclick="goTopAnimate();"><img width="45px" src="http://image2.chinatelecom-ec.com/client/wap/common/images/icon_backTop.png" /></div>';
...
function goLink(linkType,Link,backLink){
	if(linkType == '2') {
		window.location.href ="objc://goLink?linkType="+encodeURIComponent(linkType)+"&Link="+Link+"&backLink="+encodeURIComponent(backLink);
	} else {
...
(function(){
	...
	orderDetail = function(options){
		...
		_othis.settings = {
			...
			divBgImg: 'http://image2.chinatelecom-ec.com/client/wap/common/images/line_v2.png',		//图片地址
			Separator: '：',															//分隔符
...
(function(){
	Popup = function(opt){
		this.opt = {
			...
			titleImgUrl:['http://image2.chinatelecom-ec.com/client/wap/common/images/icon_qr.png',		//笑脸
			             'http://image2.chinatelecom-ec.com/client/wap/common/images/icon_cry.png'],	//哭脸
			titlecla:'common_Smile',	//笑脸样式
			//弹出框关闭按钮
			closeUse:true,				//是否展示关闭按钮
			closeImgUrl:'http://image2.chinatelecom-ec.com/client/wap/upgradePath/images/up_closebtn.png',//关闭图片
		...
		var html = '\
			<div class="onlineRetailersIndex_mask">\
				<div class="onlineRetailersIndex_content">\
					<ul class="clerafix">\
						<li><img src="http://image2.chinatelecom-ec.com/client/wap/selfOnlineRetailers/images/cry.png"/></li>\
						<li>\
							<img src="http://image2.chinatelecom-ec.com/client/wap/selfOnlineRetailers/images/talk.png"/>\
							<p class="clerafix">\
